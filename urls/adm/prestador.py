...
bp_prest = Blueprint('prest', __name__, url_prefix='/adm/prest')
...
@bp_prest.route('/consultar')  # /adm/emp/consultar
def consultar():
    dao_setor = SetorDAO()
    setores = dao_setor.read_by_filters([('sts_setor', '=', 'A')])
    return render_template('adm/prest/consultar.html', prestadores=[], setores=setores, filtro_usado='')

@bp_prest.route('/roda_consultar', methods=['POST'])  # /adm/emp/roda_consultar
def roda_consultar():
    ...
    return render_template('adm/prest/consultar.html', prestadores=prestadores, setores=setores, filtro_usado=filtro_usado)
